...
def train(epochs=1, batch_size=8):
    ...
    downscale_factor = 4 #change downscale factor as per need
    ...
    for e in range(1, epochs+1):
        ...
        if e == 1 or e % 5 == 0:
            plot_generated_images(e, generator)#save tested image after every 5 epoch
        if e % 100 == 0:#save model after every 100 epochs
            generator.save('./output/gen_model%d.h5' % e)
            discriminator.save('./output/dis_model%d.h5' % e)
            gan.save('./output/gan_model%d.h5' % e)

train(200,4)#pass epochs and batch size as parameter
